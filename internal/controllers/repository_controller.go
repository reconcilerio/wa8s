...
func RepositoryKeychain() reconcilers.SubReconciler[registriesv1alpha1.GenericRepository] {
	return &reconcilers.SyncReconciler[registriesv1alpha1.GenericRepository]{
		...
		Sync: func(ctx context.Context, resource registriesv1alpha1.GenericRepository) error {
			keychain, err := registry.KeychainForRepo(ctx, resource)
			if err != nil {
			...
			controllers.RepositoryKeychainStasher.Store(ctx, keychain)
